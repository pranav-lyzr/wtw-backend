...
def create_pension_prompt(user_message: str, user_profile: dict):
    ...
    pension_data = user_profile.get('retirement_data', [])
    ...
    - Pension data: {json.dumps(pension_data)}
    ...
    for age in years:
        ...
        if age >= 62:
            ss_value = user_profile['social_security_base'] * \
                      (1 + user_profile['inflation']) ** (age - 62)
...
async def chat_pension(request: ChatRequest):
    ...
    try:
        ...
        try:
            ...
            if json_match:
                ...
                try:
                    ...
                    # If there's no text before JSON, check if there's text after JSON
                    if not text_before_json:
                        parts = api_response["response"].split('```')
                        if len(parts) > 2:  # There might be text after the JSON block
                            text_after_json = parts[2].strip() if len(parts) > 2 else ""
                            text_response = text_after_json
                        else:
                            # Use text_response from structured response or generate a default response
                            text_response = structured_response.get("text_response", "")
                    else:
            ...
            try:
                ...
                if isinstance(parsed_chart_data, list):
                    ...
                    chart_data = {
                        ...
                        "type": "line_chart",  # or determine type from data structure
        ...
        response = ChatResponse(
            ...
            session_id=request.session_id,
            chart_data=chart_data,  # This will be None or a properly formatted dict
            contains_chart=contains_chart
        )
...
async def chat_retirement(request: ChatRequest):
    ...
    try:
        # Call Lyzr API for retirement
        logger.info("Calling Lyzr API for retirement planning")
        api_response = await call_lyzr_api(
            agent_id=RETIREMENT_AGENT_ID,
            session_id=request.session_id,
            user_id=request.user_id,
